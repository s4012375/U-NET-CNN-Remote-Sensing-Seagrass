...
MODEL_CONFIGS = {
    ...
    '5':[20,
        ...
        ['20190627', '20170717']]# No-retraining and optimisation with individual 60% training for each tile and testing on 40%
}
...
# Evaluates each patch in a tile
def evaluate_tile(val_preds, val_target_img_paths, average_precision, tile, stage):
    ...
    # Counts the number of classes which were valid and a sum of how many were added together so it can be averaged
    ...
    # Averages for valid class-wise statistics
    for i in range(0, len(CLASSES)):
        if (count_f1[i] != 0):
    ...
    # Returns a flag to the model averaged for valid classwise statistics (0 = all np.nan, 1 = one or more 0-1 values)
    for i in range(0, len(CLASSES)):
        if (count_f1[i] > 0):
              valid_f1s[i] = 1
    for i in range(0, len(CLASSES)):
...
# Evaluates the model by averaging for each tile
def evaluate_model(image_paths, target_paths, model,stage):
    ...
    # Averages for all classes if there is at least one valid statistic (not np.nan)
    for i in range(0, len(CLASSES)):
        if (count_all_f1[i] != 0):
...
##    base_model = u_net.train_base_model(train_dataset, valid_dataset, base_model, MODEL) # Use these two is continuing to train
    ...
    ##  --- TESTS ON OWN TRAINING TILES IF THE MODEL WAS STOPPED EARLY ---
##    image_paths, target_paths = ds.get_paths(model_configs[MODEL][1])
