...
def print_full_region_images():
    model = u_net.get_full_trained_resnet_model('full_resnet_checkpoint_model_%s.keras'%(MODEL)) # Gets the model in question
    for region, size in SIZE.items():
        tci_paths, target_paths = getPaths(region) # Gets all tiles
